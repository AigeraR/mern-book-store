    ...
    return (
        <div className="p-4 items-center">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                {books.map((book) => (
                    <div key={book._id} className="border p-4 rounded-lg flex flex-col">
                        ...
                        <p className="text-gray-400 text-xs sm:text-sm md:text-base">{book.category}</p>
                        <h2 className="text-sm sm:text-sm md:text-md font-semibold">{book.title}</h2>
                        <p className="text-gray-400 text-xs sm:text-sm md:text-base font-bold">{book.author}</p>
                    </div>
                ))}
            </div>
        </div>
